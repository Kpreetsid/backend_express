...
export function matchesNow(job: IJob, now: Date): boolean {
  ...
  switch (job.frequency) {
    ...
    case "custom-cron": {
      ...
      try {
        const interval = cronParser.parseExpression(job.cronExpression, { tz: zone, currentDate: dt.toJSDate() });
      ...
      case "monthly": {
        ...
        for (let m = 0; m < 24 && !placed; m++) {
          for (const d of days) {
            ...
            if (c > cursor && c.isValid) { results.push(c.toJSDate()); cursor = c; placed = true; break; }
          }
