...
class FloorMapService {
  ...
  async getCoordinates (match: any, account_id: any): Promise<any> {
    ...
    return await Promise.all(
      floorMaps.map(async (item: any) => {
        const childLocations = await this.getAllChildLocationsRecursive([`${item.locationId._id}`]);
  ...
  async getAllChildLocationsRecursive (parentIds: any): Promise<any> {
    let childIds: string[] = [];
    for (const parentId of parentIds) {
      ...
      if (children?.length > 0) {
        ...
        const grandChildren = await this.getAllChildLocationsRecursive(childrenIds);
  ...
  async insertFloorMap (body: any, account_id: any, user_id: any): Promise<any> {
  ...
  async insertCoordinates (body: any, account_id: any, user_id: any): Promise<any> {
