...
export const userAuthentication = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    ...
    const match: any = { $or: [{ username: loginName }, { email: loginName.toLowerCase() }], user_status: 'active' };
    const user: IUser | null = await UserModel.findOne(match).select('+password');
    if (!user) {
...
=======
    ...
    const analysisFeature = await analysisFeatureService.getOrCreateFeatureData(user.account_id, user._id)
...
export const createAuthenticationByToken = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    ...
    const external_user = await UserModel.findOne({ email: normalizedEmail, user_status: 'active', isVerified: true });
    if (!external_user) {
      throw Object.assign(new Error('User data not found'), { status: 404 });
    }
    const external_token = generateExternalAccessToken({ email: normalizedEmail, org_id: external_user.account_id, isExternal: false, isInternal: true });
...
export const userAuthenticationByToken = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    ...
    const decoded = (req as any).externalTokenPayload || decryptToken(external_token);
    ...
    const userDetails = await UserModel.findOne({
      ...
      account_id: helperService.validateObjectId(org_id),
      user_status: 'active',
      isVerified: true
    });
    if (!userDetails) {
    ...
    await userTokenData.save();
    const safeRedirectPath = isSafeInternalRedirect(redirectPath)
      ? redirectPath
      : '/dashboard';
    const analysisFeature = await analysisFeatureService.getOrCreateFeatureData(userDetails.account_id, userDetails._id)
    res.status(200).json(
      {
        status: true,
        message: 'Login successful',
...
export const assertPasswordNotExpired = (user: Pick<IUser, 'passwordExpiredAt'>): void => {
  if (!user.passwordExpiredAt) {
    return;
  }
  const expiryBoundary = new Date();
  expiryBoundary.setMonth(expiryBoundary.getMonth() - 3);
  if (new Date(user.passwordExpiredAt) < expiryBoundary) {
    throw Object.assign(
      new Error('Your password has expired. Please reset your password to continue.'),
      { status: 403 }
    );
  }
};
