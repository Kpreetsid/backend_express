...
export const createLocationsReport = async (body: any) => {
  const { account_id, location_id, user_id } = body;
  ...
  if (!assets || assets.length === 0) {
    throw Object.assign(new Error('No assets found in the specified locations'), { status: 404 });
  }
  const reportList = await Promise.all(assets.map(async (asset: any) => {
    const [latestReport] = await ReportAsset.find({ top_level_asset_id: asset._id, accountId: account_id }).sort({ _id: -1 }).limit(1);
  ...
  const sub_location_data = Object.values(subLocationMap);
