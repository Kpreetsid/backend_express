...
import { companyService } from '../masters/company/company.service';
import { TokenModel } from '../models/userToken.model';
import { decryptToken } from './externalToken';

export { decryptToken, encryptToken, generateExternalAccessToken } from './externalToken';
...
export const clearAuthSessionCacheForUser = (userId: string): void => {
  ...
  for (const [cacheKey, session] of authSessionCache.entries()) {
    if (String(session.user?._id || session.user?.id || '') === normalizedUserId) {
...
export const verifyEncryptedToken = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    (req as any).externalTokenPayload = payload;
    next();
  } catch (error: any) {
    next(error);
  }
