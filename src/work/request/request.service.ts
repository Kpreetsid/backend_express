...
class RequestService {
  ...
  async getAllRequests (match: any): Promise<IWorkRequest[]> {
    match.visible = true;
    const populateList = [
      { path: "location_id", model: "Schema_Location", match: { visible: true }, select: "id location_name location_type" },
      { path: "asset_id", model: "Schema_Asset", match: { visible: true }, select: "id asset_name asset_type" },
