...
export async function processJobsTick() {
    try {
        const now = new Date();
        console.log(`[JOB_RUN] Starting job tick at ${now.toISOString()}`);

        if (mongoose.connection.readyState !== 1) {
            console.log("[JOB_RUN] MongoDB not connected. Skipping job tick.");
            return;
        }
        ...
        for (const schedule of schedules) {
            try {
                console.log(`[JOB_RUN] Processing schedule: ${schedule.title} (${schedule.schedule_mode})`);



                // For testing, let's always execute if the schedule is found
...
async function executeSchedule(schedule: any, executionDate: Date) {
    try {
...
export async function triggerScheduleManually(scheduleId: string) {
    try {
        ...
        await executeSchedule(schedule, new Date());

        return { success: true, message: `Schedule ${schedule.title} executed successfully` };
    } catch (error: any) {
        console.error("[JOB_RUN ERROR] Failed during job tick:", error.message || error);
    }
}

export async function updateNextRun(job: any) {
