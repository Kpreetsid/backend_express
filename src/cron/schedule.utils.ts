...
export function inRange(job: any, now: Date): boolean {
    if (job.startDate && now < job.startDate) return false;
    if (job.endDate && now > job.endDate) return false;
    return true;
}

export function matchesNow(job: any, now: Date): boolean {
...
export function computeNextRuns(job: any, count = 10, from = new Date()): Date[] {
    const zone = job.timezone || "Asia/Kolkata";
    const results: Date[] = [];
