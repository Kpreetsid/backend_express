...
// src/cron/templates/WorkOrderTemplateEngine.ts
...
export class WorkOrderTemplateEngine {
    ...
    static async generateWorkOrderData(schedule: any, executionDate: Date): Promise<any> {
        ...
        return {
            ...
            sop_form_id: wo.sop_form_id || new mongoose.Types.ObjectId(),
            // work_instruction: wo.workInstruction ? new mongoose.Types.ObjectId(wo.workInstruction) : new mongoose.Types.ObjectId(),

            // Repeat configuration — matches DB format
            repeat: {
                weekly: {
                    interval: schedule.schedule?.repeat?.weekly?.interval ?? 1,
            ...
            // Tasks — let Mongoose auto-generate _id
            tasks: (wo.tasks || []).map((task: any) => ({
                ...
                options: (task.options || []).map((opt: any) => ({
                    key: opt.key,
                    value: String(opt.value) // Mongoose will auto-generate _id
                }))
            })),

            // Parts — let Mongoose auto-generate _id
            ...
            files: [],
            work_request_id: wo.work_request_id || new mongoose.Types.ObjectId(),
            visible: true,
            createdBy: schedule.createdBy,
            createdFrom: wo.createdFrom || "Preventive" // match your DB default
        };
    }
